...
- Decide and verify the next product phase after Phase 2F-G/G1. Basic local AI generation/review is accepted, and the next render-quality acceptance target is defined in `docs/phase-2g-render-quality-acceptance.md`; remaining choices are production deployment hardening, implementing the Phase 2G verifier, or cloud/provider-backed AI generation hardening.
