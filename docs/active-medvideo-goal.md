...
- Phase 2G quality target: `docs/phase-2g-render-quality-acceptance.md` defines the next scene-level render-quality acceptance target before any new renderer or fulfillment changes.
- Phase 2G quality evidence: `data/review-artifacts/24c46f1f-fb5e-4aad-bdb6-ad61a7f2ca99/phase-2g-render-quality/render_quality_report.json` and `quality_contact_sheet.png` are generated by `npm.cmd run verify:phase2g-render-quality`. Current verdict is `NEEDS_HUMAN_REVIEW_OR_SMALL_FIXES`: technical checks pass, but scene-level quality does not fully pass.
...
- Decide and verify the next product phase after Phase 2F-G/G1. Basic local AI generation/review is accepted, and Phase 2G render-quality evidence now exists; remaining choices are production deployment hardening, fixing the concrete Phase 2G failed scenes, or cloud/provider-backed AI generation hardening.
...
- Added `npm.cmd run review:phase2g-clinical`, which packages the four remaining clinical/anatomy scenes into `clinical_human_review_packet.json`, `clinical_human_review_packet.md`, `medical_safety_review.md`, `human_review_prompt.md`, and extracted frame evidence for human medical/design approval.
