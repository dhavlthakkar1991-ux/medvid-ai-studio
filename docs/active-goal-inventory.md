...
- AI asset generation/review workflow,
...
| Next render-quality phase | `pending_next_phase` | Latest benchmark MP4 is ffprobe-valid and persisted; additional visual polish beyond Phase 2F-G/G1 is outside the current audit. | A concrete visual acceptance target is defined for the next benchmark, with before/after evidence points and non-goals. | Define the next visual acceptance target before changing renderer or layout behavior. | Render benchmark MP4, capture frame evidence/contact sheet, verify Studio persistence, ffprobe, output URL, and qualitative pass/fail criteria. |
| AI asset generation and review workflow | `pending_next_phase` | Per-requirement prompts and upload/replace controls are verified; automatic HeyGen/AI asset generation is not implemented or verified by this audit. | Provider credentials and cost/safety boundaries are explicit, and generated assets remain review-gated rather than auto-approved. | Design and verify provider-backed generation without bypassing human review, licensing, or Studio-as-Director constraints. | Generate one asset from a requirement row, store prompt/provider/model/cost/result metadata, show it in Review Assets, approve or reject through human review, then prove Manifest/RenderSpec mapping only after approval. |
