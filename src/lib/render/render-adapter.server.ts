...
import { validateRenderSpec } from "./render-validation";
...
export async function adapterCreateRender(
  ...
  if (!validation.ok) {
    const detail = validation.issues
      ...
      .join("; ");
    throw new Error(`RenderSpec validation failed: ${detail || "unknown error"}`);
  }
  ...
  await Promise.all([
    sb.from("render_provider_jobs").insert({
      ...
      logs: [
        { at: new Date().toISOString(), level: "info", msg: `Created render via ${impl.name}` },
      ],
    }),
    sb
      .from("render_jobs")
...
export async function adapterPoll(
  ...
  const providerRow =
    pj.render_providers ??
    (await sb.from("render_providers").select("*").eq("id", pj.provider_id).maybeSingle()).data;
  const impl = getProviderByType(providerRow.provider_type as ProviderType);
  ...
  await sb
    ...
    .update({
      ...
      response_payload: { ...(pj.response_payload ?? {}), last_report: report },
    })
    .eq("id", pj.id);
